...
async function sendDiscordNotification(submission) {
  ...
  const body = JSON.stringify({
    embeds: [{
      title:     `🔔 New Submission — ${isPartner ? '🤝 Business Partner' : '🎯 Skilled Individual'}`,
      color:     isPartner ? 0xFFD000 : 0x26C7D9,
  ...
  try {
    ...
    console.log('[Discord]', res.ok ? '✅ Sent' : '❌ Failed: ' + res.status);
...
app.post('/api/submit/individual', submitLimiter, async (req, res) => {
  try {
    ...
    if (!skill_category?.trim())   return res.status(400).json({ error: 'Please select your skill.' });
    ...
    const submission = {
      ...
      full_name: sanitize(full_name), email: validator.normalizeEmail(email),
      phone: phone ? sanitize(phone) : null,
      skill_category: sanitize(skill_category), about_yourself: sanitize(about_yourself),
      ip_address: req.ip, user_agent: req.headers['user-agent'] || null
    };

    const { data, error } = await supabase.from('submissions').insert(submission).select().single();
    if (error) { console.error('[DB]', error); return res.status(500).json({ error: 'Failed to save. Please try again.' }); }

    await Promise.allSettled([
      sendDiscordNotification(data), sendTeamEmail(data), sendConfirmationEmail(data)
    ]);

    return res.status(201).json({ success: true, message: "Application received! We'll reach out within 48 hours.", id: data.id });
  } catch (e) {
...
app.post('/api/submit/partner', submitLimiter, async (req, res) => {
  try {
    ...
    if (!division_interest?.trim()) return res.status(400).json({ error: 'Please select a division.' });
    ...
    const submission = {
      ...
      full_name: sanitize(full_name), email: validator.normalizeEmail(email),
      phone: phone ? sanitize(phone) : null,
      business_name: sanitize(business_name), division_interest: sanitize(division_interest),
    ...
    return res.status(201).json({ success: true, message: "Partnership inquiry received! We'll reach out within 48 hours.", id: data.id });
  } catch (e) {
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    if (!adminUser) return res.status(401).json({ error: 'Account not found. Contact Jose.' });
    if (!adminUser.is_active) return res.status(403).json({ error: 'Account deactivated. Contact Jose.' });

    await supabase.from('admin_users').update({ last_login: new Date().toISOString() }).eq('id', adminUser.id);
    await supabase.from('audit_logs').insert({ admin_id: adminUser.id, admin_name: adminUser.name, action: 'LOGIN', details: { ip: req.ip } });
...
app.post('/api/auth/logout', requireAuth, async (req, res) => {
  await supabase.from('audit_logs').insert({ admin_id: req.user.id, admin_name: req.user.name, action: 'LOGOUT' }).catch(() => {});
...
// ============================================================
// ROUTES — ADMIN SUBMISSIONS (all before :id routes)
// ============================================================
app.get('/api/admin/submissions', requireAuth, async (req, res) => {
  try {
    const { type, status, search, page = 1, limit = 20 } = req.query;
    console.log('[Submissions] Fetching...', { type, status, page });
    ...
    return res.json({ submissions: data || [], total: count || 0, page: Number(page), pages: Math.ceil((count || 0) / limit) });
  } catch (e) {
...
// ============================================================
// ROUTES — TEST (remove before going public)
// ============================================================
app.get('/api/test-discord', async (req, res) => {
  try {
    await sendDiscordNotification({
      type: 'skilled_individual', full_name: 'Test User', email: 'test@test.com',
      phone: '09123456789', skill_category: 'Web / App Development',
      about_yourself: 'Test notification from TEAM | ProjectOneUSA API'
    });
    return res.json({ success: true, message: 'Test fired — check Discord.' });
