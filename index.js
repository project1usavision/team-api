...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    // Use the verified email from Supabase Auth
...
app.get('/api/admin/submissions', requireAuth, async (req, res) => {
  try {
    ...
    console.log('[Submissions] Query params:', { type, status, search, page, limit });
    ...
    console.log('[Submissions] Result count:', count, 'error:', error?.message);
    if (error) throw error;
    ...
    return res.status(500).json({ error: 'Failed to fetch submissions.', detail: e.message });
  }
});
